...
//---------------------------------------------------------
// Experimental block-based capture mode
#ifdef __SYNTHESIS__
  #define CCS_BLOCK(fn) fn
    ...
    // Hack for the fact that assert.h may redefine the assert macro
    // out from under us, reinclude ac_assert.h here if it appears
    // to have been included already
