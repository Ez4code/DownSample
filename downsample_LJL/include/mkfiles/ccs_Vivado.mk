...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# Work on supporting gate-level netlist simulation in SCVerify
ifeq "$(STAGE)" "gate"

# Caller must set
#  XV_SRC = name of Xilinx Vivado command
#
# Available targets:
#    build - synthesis
...
# Translate the flow option XilinxVivado/OutNetlistFormat into either "v" or "vhdl"
ifeq "$(XilinxVivado_OutNetlistFormat)" "verilog"
OutNetlistFormat2 = v
else
ifeq "$(XilinxVivado_OutNetlistFormat)" "vhdl"
OutNetlistFormat2 = vhdl
endif
endif

ifneq "$(XilinxVivado_Path)" ""
XV_CMD = $(XilinxVivado_Path)/$(XilinxVivado_ShellExe)
else
XV_CMD = $(XilinxVivado_ShellExe)
endif

# Build gate netlist dependency file based on user's output netlist option
ifeq "$(OutNetlistFormat2)" "vhdl"
GATE_VHDL_DEP = $(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2)/$(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2)ts
endif
ifeq "$(OutNetlistFormat2)" "v"
GATE_VLOG_DEP = $(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2)/$(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2)ts
endif
...
# Path to TCL script to parse synthesis output log
RTL_FLOW_FNAME = $(MGC_HOME)/pkgs/sif/userware/En_na/flows/app_$(RTLTOOL).flo

# TRANSLATION RULES
#===============================================================================
# Synthesize RTL to HDL gate netlist
$(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2) : $(RTL_SYN_SCRIPT) $(RTL_NETLIST_FNAME)
	@-$(ECHO) "============================================"
	@-$(ECHO) "Synthesizing $@, logging to $(RTL_SYN_SCRIPT).log"
	$(XV_CMD) -f $< 1> $(RTL_SYN_SCRIPT).log

$(RTL_SYN_SCRIPT).log: $(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2) 
...
#	$(XV_CMD) -f $< 1> $<.log
#	cd $(SYN_DIR) && $(XV_CMD) -f ../$< 1> ../$<.log

#===============================================================================
# Support for gate-level netlist simulation
ifneq "$(RTLTOOL)" ""
ifeq "$(STAGE)" "gate"
.PHONY: build
build: $(XilinxVivado_OutNetlistName)$(suffix $(RTL_NETLIST_FNAME)).$(OutNetlistFormat2) $(HLD_CONSTRAINT_FNAME)
...
helptext:
	...
	@-$(ECHO) "   build    Run Vivado synthesis and compile the gate netlist"
	@-$(ECHO) "   sim      Run Vivado synthesis, compile the gate netlist and simulate"
	@-$(ECHO) "   simgui   Run Vivado synthesis, compile the gate netlist and simulate interactively"
...
dumpvars:
	@-$(ECHO) "   XilinxVivado flow options:"
	@-$(ECHO) "     SearchPath            = $(XilinxVivado_SearchPath)"
	@-$(ECHO) "     OutNetlistName        = $(XilinxVivado_OutNetlistName)"
	@-$(ECHO) "     OutNetlistFormat      = $(XilinxVivado_OutNetlistFormat)"
